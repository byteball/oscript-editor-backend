...
router.post('/validate', asyncMiddleware(async (req, res, next) => {
	...
	try {
		...
		res.status(200).send(body ? 'AA validated, complexity = ' + body.complexity + ', ops = ' + body.count_ops : 'AA validated')
	} catch (error) {
		let status, body
		if (error instanceof ValidationError) {
			status = 400
...
router.post('/is-duplicate', asyncMiddleware(async (req, res, next) => {
	const data = JSON.parse(req.body.data)
	try {
		await aaService.checkAgentAlreadyDeployed(data)
		res.status(200).send({ isDuplicate: false })
	} catch (error) {
		let status, body
		if (error instanceof AgentAlreadyDeployedError) {
			status = 200
			body = { error: error.message, isDuplicate: true }
		} else {
			status = 500
			body = { error: error.message || error, errorCode: ErrorCodes.INTERNAL_ERROR }
		}
